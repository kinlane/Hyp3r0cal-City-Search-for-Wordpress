...
<div id="primary" style="border: 0px solid #000; width: 650px;">
	...
	// page_id
	if(isset($_REQUEST['page_id'])){ $page_id = $_REQUEST['page_id'];} elseif(isset($_POST['page_id'])){$page_id = $_POST['page_id']; }
	...
	// where
	if(isset($_REQUEST['where'])){ $where = $_REQUEST['where'];} elseif(isset($_POST['where'])){$where = $_POST['where']; } else { $where=''; } 
	...
	if($id!='') 
		...
		<strong>
			<a href="?page_id=<?php echo $page_id; ?>"><?php echo get_option('directory_label'); ?></a> /
			<?php if($where!=''&&$what!='') {?>
				<a href="?page_id=<?php echo $page_id; ?>&what=<?php echo $what; ?>&where=<?php echo $where; ?>"><?php echo $where;?></a> /
			<?php } ?>
			<?php if($where!=''&&$what!='') {?>
				<a href="?page_id=<?php echo $page_id; ?>&what=<?php echo $what; ?>&where=<?php echo $where; ?>"><?php echo $what;?></a>
			<?php } ?>
		</strong>
	...
	elseif($where!=''&&$what!='')
		...
		<script type="text/javascript">
			function performCitySearch()
				...
				what = document.getElementById('text-search-what').value;
				where = document.getElementById('text-search-where').value;
				
				location.href = "?page_id=<?php echo $page_id;?>&what=" + what + "&where=" + where;
				}
		</script>
		<form action="#" method="GET">
			<input type="hidden" name="page_id" id="page_id" value="<?php echo $page_id; ?>" />
			<label>&nbsp;&nbsp;<strong>What:</strong></label>
			<input type="text" id="text-search-what" size="30" value="Coffee">
			<label>&nbsp;&nbsp;<strong>Where:</strong></label>
			<input type="text" id="text-search-where" size="30" value="West Hollywood, CA">		
			<button type="button" class="btn" id="searchButton" onclick="performCitySearch();">Search</button>
		</form>
		</center>
		<br />
		<strong>
			<a href="#"><?php echo get_option('directory_label'); ?></a> /
		</strong>
		<br /><br />
		<?php echo get_option('directory_text'); ?>
		<br />
		<br />
		<?php
		}
