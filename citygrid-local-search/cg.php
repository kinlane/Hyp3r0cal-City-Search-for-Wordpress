<?php
/*
Plugin Name: CityGrid Hyp3rL0cal Search
Plugin URI: 
Description: CityGrid Hyp3rL0cal Search
Version: 0.1.1.1
...
define( 'CG_PLUGIN_DIRECTORY', 'citygrid-local-search');
...
function cg_create_menu() {
	...
	if ( ! $the_page ) {
	    ...
	    $_p['post_content'] = "<p>Welcome to the CityGrid Hyp3rL0cal Search</p>";
...
function catch_plugin_template($template) {
    ...
    if( is_page_template('cg-search.php') )
        ...
        $template = WP_PLUGIN_DIR . '/citygrid-local-search/cg-search.php';
    	}
...
function cg_register_settings() {
	...
	if(!get_option('directory_label'))
		{
		update_option( 'directory_label', 'City Search' );
		}
		
	//register settings
	register_setting( 'cg-settings-group', 'directory_text' );
	
	if(!get_option('directory_text'))
		{
			
		$directory_text = '<p>This City Search allows you to search for businesses in any city in the United States</p>';
		$directory_text .= '<p>Using the form above you can:</p>';
		$directory_text .= '<ol>';
		$directory_text .= '<li>specify a "what", or any type of business you wish to find.</li>';
		$directory_text .= '<li>specif a "where", which could be cities, neighborhoods, zip codes, metro areas, addresses and intersections.</li>';
		$directory_text .= '</ol>';
		$directory_text .= '<p>After each search you can browse businesses and see detail, images, reviews and links to more information for each location.</p>';

		update_option( 'directory_text', $directory_text );
		}			
